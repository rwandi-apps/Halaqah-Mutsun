...
const formatRangeDisplay = (raw: string | undefined): string => {
  ...
  // Deteksi format "Surah: Ayat - Surah: Ayat"
  ...
  if (parts.length === 2) {
    ...
    if (p1.length === 2 && p2.length === 2) {
      ...
      // Jika surat sama, ringkas menjadi "Surah: Ayat1-Ayat2"
...
const getStoredOrCalculatedResult = (report: Report, category: 'tahfizh' | 'tilawah') => {
  ...
  if (target.result && target.result !== '-' && target.result !== '0H 0B') {
    // Pastikan singkatan diubah menjadi teks penuh
  ...
  return (
    <div className="space-y-6 max-w-full mx-auto pb-12 px-2">
      ...
      <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-gray-100 grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <select value={filterType} onChange={e => {setFilterType(e.target.value); setFilterMonth(e.target.value === 'Laporan Semester' ? 'Ganjil' : 'Desember');}} className="p-2.5 border border-gray-200 rounded-xl bg-gray-50 text-sm font-bold outline-none"><option value="Laporan Bulanan">Laporan Bulanan</option><option value="Laporan Semester">Laporan Semester</option></select>
        <select value={filterMonth} onChange={e => setFilterMonth(e.target.value)} className="p-2.5 border border-gray-200 rounded-xl bg-gray-50 text-sm font-bold outline-none">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse whitespace-nowrap">
            ...
            <tbody className="divide-y divide-gray-100 text-[9px]">
              {isLoading ? (
                <tr><td colSpan={12} className="px-6 py-20 text-center"><Loader2 size={32} className="text-primary-500 animate-spin mx-auto" /></td></tr>
