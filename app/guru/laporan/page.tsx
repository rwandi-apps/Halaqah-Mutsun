...
import { BookOpen, Book, Plus, Minus, Database, UserCheck, Layers, Loader2, Calendar, MessageSquare, Users } from 'lucide-react';
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await saveSDQReport({
        ...
        tilawah: { 
          method: tilawahIndiv.method, 
          individual: makeRange(tilawahIndiv),
          classical: makeRange(tilawahKlas),
          result: tlResultStr 
        },
        tahfizh: { 
          individual: makeRange(tahfizhIndiv),
          classical: makeRange(tahfizhKlas),
          result: tfResultStr 
        },
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-12 px-2">
      ...
      <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <select value={month} onChange={(e) => setMonth(e.target.value)} className="p-2 border rounded-xl bg-white text-sm">{MONTH_LIST.map(m => <option key={m} value={m}>{m}</option>)}</select>
      </div>

      <div className="grid grid-cols-1 gap-6">
        {/* KLASIKAL SECTION (SEKARANG DI ATAS) */}
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 space-y-6 border-t-4 border-t-blue-500">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 pt-4 border-t border-gray-50">
            <div className="space-y-4">
              ...
              <InputRow label="DARI"><SourceSelect value={tahfizhKlas.fs} onChange={(v) => setTahfizhKlas({...tahfizhKlas, fs: v})} method="Al-Quran" /><CounterInput label="Ayat" value={tahfizhKlas.fv} onChange={(v) => setTahfizhKlas({...tahfizhKlas, fv: v})} /></InputRow>
              <InputRow label="SAMPAI"><SourceSelect value={tahfizhKlas.ts} onChange={(v) => setTahfizhKlas({...tahfizhKlas, ts: v})} method="Al-Quran" /><CounterInput label="Ayat" value={tahfizhKlas.tv} onChange={(v) => setTahfizhKlas({...tahfizhKlas, tv: v})} /></InputRow>
            </div>
            <div className="space-y-4">
              ...
              <InputRow label="DARI"><SourceSelect value={tilawahKlas.fs} onChange={(v) => setTilawahKlas({...tilawahKlas, fs: v})} method={tilawahKlas.method} /><CounterInput label={tilawahKlas.method === 'Al-Quran' ? 'Ayat' : 'Hal'} value={tilawahKlas.fv} onChange={(v) => setTilawahKlas({...tilawahKlas, fv: v})} /></InputRow>
              <InputRow label="SAMPAI"><SourceSelect value={tilawahKlas.ts} onChange={(v) => setTilawahKlas({...tilawahKlas, ts: v})} method={tilawahKlas.method} /><CounterInput label={tilawahKlas.method === 'Al-Quran' ? 'Ayat' : 'Hal'} value={tilawahKlas.tv} onChange={(v) => setTilawahKlas({...tilawahKlas, tv: v})} /></InputRow>
            </div>
          </div>
        </div>

        {/* INDIVIDU SECTION (SEKARANG DI BAWAH) */}
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 space-y-6 border-t-4 border-t-emerald-500">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 pt-4 border-t border-gray-50">
            <div className="space-y-4">
              ...
              <InputRow label="DARI"><SourceSelect value={tahfizhIndiv.fs} onChange={(v) => setTahfizhIndiv({...tahfizhIndiv, fs: v})} method="Al-Quran" /><CounterInput label="Ayat" value={tahfizhIndiv.fv} onChange={(v) => setTahfizhIndiv({...tahfizhIndiv, fv: v})} /></InputRow>
              <InputRow label="SAMPAI"><SourceSelect value={tahfizhIndiv.ts} onChange={(v) => setTahfizhIndiv({...tahfizhIndiv, ts: v})} method="Al-Quran" /><CounterInput label="Ayat" value={tahfizhIndiv.tv} onChange={(v) => setTahfizhIndiv({...tahfizhIndiv, tv: v})} /></InputRow>
            </div>
            <div className="space-y-4">
              ...
              <InputRow label="DARI"><SourceSelect value={tilawahIndiv.fs} onChange={(v) => setTilawahIndiv({...tilawahIndiv, fs: v})} method={tilawahIndiv.method} /><CounterInput label={tilawahIndiv.method === 'Al-Quran' ? 'Ayat' : 'Hal'} value={tilawahIndiv.fv} onChange={(v) => setTilawahIndiv({...tilawahIndiv, fv: v})} /></InputRow>
              <InputRow label="SAMPAI"><SourceSelect value={tilawahIndiv.ts} onChange={(v) => setTilawahIndiv({...tilawahIndiv, ts: v})} method={tilawahIndiv.method} /><CounterInput label={tilawahIndiv.method === 'Al-Quran' ? 'Ayat' : 'Hal'} value={tilawahIndiv.tv} onChange={(v) => setTilawahIndiv({...tilawahIndiv, tv: v})} /></InputRow>
            </div>
          </div>
        </div>
