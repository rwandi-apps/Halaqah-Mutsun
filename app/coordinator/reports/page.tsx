...
export default function CoordinatorReportsPage() {
  ...
  // Helper formatting copied from Guru View for consistency
  ...
  const handleExportCSV = () => {
    ...
    const rows = filteredReports.map((r, i) => {
      ...
      return [
        ...
        "Status", // Placeholder for actual text logic if needed
        r.notes
      ].map(v => `"${v}"`).join(",");
    });
    ...
    a.href = url;
    a.download = `Supervisi_Laporan_${new Date().toISOString().split('T')[0]}.csv`;
    a.click();
  };
  ...
  return (
    <div className="space-y-6 max-w-full mx-auto pb-12">
      ...
      {/* Table Section - Mapped from Guru Lihat Laporan Format */}
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden mx-2">
        <div className="overflow-x-auto custom-scrollbar">
          <table className="w-full text-left border-collapse whitespace-nowrap">
            ...
            <tbody className="divide-y divide-gray-100 text-sm">
              {isLoading ? (
                <tr><td colSpan={11} className="px-6 py-12 text-center text-gray-500">Memuat data supervisi...</td></tr>
                ...
                filteredReports.map((report, idx) => {
                  ...
                  return (
                    <tr key={report.id} className={`hover:bg-gray-50/50 transition-colors group ${idx % 2 === 0 ? 'bg-white' : 'bg-gray-50/30'}`}>
                      ...
                      <td className="px-4 py-3 align-middle border-r border-gray-50">
                        <div className="font-bold text-gray-900">{report.studentName}</div>
                        <div className="text-[10px] text-gray-400">{report.className}</div>
                      </td>
        ...
        <div className="px-6 py-3 border-t border-gray-100 bg-gray-50/50 flex justify-between items-center text-[10px] text-gray-400 font-bold uppercase tracking-widest">
           <div className="flex gap-4">
              <span className="flex items-center gap-1"><BookOpen size={12} className="text-primary-500"/> Data Individual</span>
              <span className="flex items-center gap-1"><Layers size={12} className="text-teal-600"/> Data Klasikal</span>
           </div>
